...
const Navbar = ({ setSearchQuery }: { setSearchQuery: (query: string) => void }) => {
        ...
        <form onSubmit={handleSearch} className="flex-1 max-w-[800px] px-4">
          <div className="relative w-3/4 mx-auto">
            <input
              ...
              className="w-full pl-5 pr-10 py-2 border rounded-3xl focus:outline-none focus:ring-2 focus:ring-orange-500 md:placeholder-gray-400 placeholder-transparent"
            />
            <button type="submit" className="absolute right-4 top-1/2 transform -translate-y-1/2">
              <Search className="h-5 w-5 text-gray-400 stroke-orange-500" />
            </button>
          </div>
        </form>
