export function getTimeDifference(unixTimestamp: number): string {
    ...
    const days = Math.floor((absDifference % (365 * 24 * 3600)) / (24 * 3600));
    ...
    if (hours > 0 || days > 0 || years > 0) result += `${hours} hour${hours > 1 ? 's' : ''}, `;
    if (minutes > 0 || hours > 0 || days > 0 || years > 0) result += `${minutes} minute${minutes > 1 ? 's' : ''}, `;
